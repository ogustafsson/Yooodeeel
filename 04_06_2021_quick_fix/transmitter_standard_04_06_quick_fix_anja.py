...
def create_header(filename):
    ...
    print("The header bits are: " + str(header[24:32]))
    return header

def convert_file_to_bits_include_header(file_name):
    with open(file_name, 'rb') as file:
        file_bytes = np.array([byte for byte in file.read()], dtype=np.uint8)
    file_bits = np.unpackbits(file_bytes)
    file_bits = np.array(file_bits, dtype=int)

    header_bits = create_header(file_name)

    return np.concatenate([header_bits, file_bits])
...
def split_sequence_into_blocks_with_pilot_tones(constellation_sequence):
  ...
  pilot_indices = np.arange(0, 1018, 8)
...
def generate_data_signal(data_file):
    ...
    print("Bit stream: " + str(bit_stream[0:8]))
    ...
    constellation_sequence = constellation_sequence[1:]
...
def generate_signal_for_transmission(data_file):
    ...
    overall_signal = np.concatenate((chirp_signal, preamble_signal, estimation_signal, data_signal, chirp_signal[::-1]))

    convert_array_to_wav(overall_signal, sample_rate, FILE_NAME_TRANSMIT)

    return overall_signal

overall_signal = generate_signal_for_transmission("data_text.txt") #this data length is used as a global constant in receiver.py
