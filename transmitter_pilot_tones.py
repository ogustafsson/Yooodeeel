...
chirp_constants = {"startFrequency": 100, "endFrequency": 10000, "duration": 0.1, "method": "logarithmic"}
...
def split_sequence_into_blocks(constellation_sequence):
    """
    Takes a QPSK constellation sequence and splits it into (N/2 - 1) length blocks
    ...
    return blocks, len(blocks) # Return array of (N/2-1) long blocks as well as the length of the constellation sequence for later use


# Create OFDM symbols and add pilot tones to each symbol.

def split_sequence_into_blocks_with_pilot_tones(constellation_sequence, pilot_index):
  #pilot_index is index at which we place pilot tone i.e. pilot_index = 10 means that there will be pilot tone at indices [1, 11, 21, ...]

  pilot_indices = list(range(1, N//2, pilot_index))
  pilot_indices = np.array(pilot_indices)

  number_of_pilot_tones = len(pilot_indices)
  number_of_data_tones = N//2 - 1 - number_of_pilot_tones
  ...
  for block in blocks_excluding_last:
    ...
    for i in range(N//2-1):
      if (np.isin(i, pilot_indices)):
        current_block = np.append(current_block, A/np.sqrt(2)*(1+1j))
      else:
...
def generate_signal_for_transmission_with_pilot_tones(data_file):
    """
    Master function for generating the signal for transmission over an audio channel
    Input: name of the file to be transmitted
    Output: writes audio .wav file to disk
    ...
    constellation_indices = map_bits_to_constellation_indices(encoded_bit_stream) # 3.1 Map encoded bit stream to constellation indices
    constellation_sequence = map_indices_to_constellation(constellation_indices) # 3.2 Map constellation indices to QPSK constellation symbols
    data_blocks, data_constellation_length = split_sequence_into_blocks_with_pilot_tones(constellation_sequence, 10) # 4. Split sequence of constellation symbols into blocks and add random padding
    data_signal = generate_data_signal(data_blocks) # 6. Generate data signal by converting frequency-domain blocks to time-domain OFDM symbols
    ...
    # Prepare signal for transmission
    overall_signal = set_up_overall_signal(chirp_signal, estimation_signal, data_signal) # 9. Set up overall signal with three components
    file_name = 'pilot_' + FILE_NAME_TRANSMIT
    convert_array_to_wav(overall_signal, sample_rate, file_name) # 10. Write to disk audio .wav file for transmisison over channel

    return data_length


data_length = generate_signal_for_transmission_with_pilot_tones("data_text.txt") #this data length is used as a global constant in receiver.py
print(data_length)
